...
open class ImageSlideshow: UIView, UIScrollViewDelegate {
    ...
    open var activityView : UIActivityIndicatorView = UIActivityIndicatorView(activityIndicatorStyle: .whiteLarge)
    ...
    func getImagesID(HouseID: [String]){
        ArrayImagesID = []
        ArrayImagesIDString = HouseID
        
        DispatchQueue.global().async(execute: {
            if !self.Allhouses
            {
                self.loadmore = self.load + 5
            }
            
            //if !self.Allhouses
            //{
                for i in self.load..<self.loadmore
                {
                ...
                DispatchQueue.main.async(execute: {
                    self.setImageInputs(self.ArrayImagesID)
                })
            //}
            
            for i in 0..<5
            {
                if self.loadmore + i == self.ArrayImagesIDString.count
                {
                    self.loadmore = self.ArrayImagesIDString.count
                    //self.Allhouses = true
                }
                
                if self.load - i == self.ArrayImagesIDString.count
                {
                    self.Allhouses = true
                }
            }
    ...
    func startParsingImages(_ codigo: String) -> URL
    {
        ...
        guard let myURL = URL(string: "http://lascasas.pt/XX/app/mobile_api.php?func=get_image_resized&image_id=" + codigo + "&width=\(width)&height=\(512/*screenHeight*/)&crop=no") else {
            print("Error: http://lascasas.pt/app/mobile_api.php?func=get_image&image_id= doesn't seem to be a valid URL")
    ...
    open func setImageInputs(_ inputs: [InputSource]) {
        self.images.append(contentsOf: inputs)
        self.pageControl.numberOfPages = self.IdHouse.count
...
//                    if Allhouses == false
//                    {
//                        self.slideshowInterval = 0
//                        loading = true
//                        let activityWidthAndHeight : CGFloat = CGFloat(30)
//                        activityView.color = self.Red_Carlo
//                        activityView.frame = CGRect(x: self.frame.size.width/2 - activityWidthAndHeight/2, y: self.frame.size.height/2 - activityWidthAndHeight/2, width: activityWidthAndHeight, height: activityWidthAndHeight)
//                        //scrollView.addSubview(self.activityView)
//                        activityView.isHidden = false
//                        activityView.startAnimating()
//                        load += 5
//                        if self.loadmore != 0 && self.loadmore == self.ArrayImagesIDString.count
//                        {
//                            self.Allhouses = true
//                        }
//                        getImagesID(HouseID: self.IdHouse)
//                    }
//                    else
//                    {
