...
//    private var overlayLayer: CAShapeLayer = CAShapeLayer()
    ...
    */
    open func open() {
        if(items.count > 0){
            ...
            overlayView.addTarget(self, action: #selector(KCFloatingActionButton.close), for: UIControlEvents.touchUpInside)
    ...
    */
    open func close() {
        if(items.count > 0){
            self.overlayView.removeTarget(self, action: #selector(KCFloatingActionButton.close), for: UIControlEvents.touchUpInside)
            UIView.animate(withDuration: 0.3, delay: 0,
    ...
    */
    ...
    fileprivate func setObserver() {
        NotificationCenter.default.addObserver(self, selector: #selector(KCFloatingActionButton.deviceOrientationDidChange(_:)), name: NSNotification.Name.UIDeviceOrientationDidChange, object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(KCFloatingActionButton.keyboardWillShow(_:)), name:NSNotification.Name.UIKeyboardWillShow, object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(KCFloatingActionButton.keyboardWillHide(_:)), name:NSNotification.Name.UIKeyboardWillHide, object: nil)
    }
    ...
    internal func keyboardWillShow(_ notification: Notification) {
        guard let keyboardSize:CGFloat = (notification.userInfo?[UIKeyboardFrameBeginUserInfoKey] as? NSValue)?.cgRectValue.size.height else { return }
